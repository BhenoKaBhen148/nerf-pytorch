...
def get_rays_np(H, W, K, c2w):
    ...
    dirs = np.stack([(i/W-1/2)*360, -(j/H-1/2)*360, -np.ones_like(i)], -1)

    # dirs = np.stack([(i-K[0][2])/K[0][0]*np.pi, -(j-K[1][2])/K[1][1]*np.pi/2, -np.ones_like(i)], -1)
    # Rotate ray directions from camera frame to the world frame
    rays_d = np.sum(c2w[:3,:3] * dirs[..., np.newaxis, :] *  np.linalg.inv(c2w[:3,:3]), -1)  # dot product, equals to: [c2w.dot(dir) for dir in dirs]
    # Translate camera frame's origin to the world frame. It is the origin of all rays.
